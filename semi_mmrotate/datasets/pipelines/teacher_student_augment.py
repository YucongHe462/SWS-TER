...
class SARToPILImage(SingleImageOperation):
    def __init__(self):
        super().__init__()
...
@ROTATED_PIPELINES.register_module()
class SARRandomGrayscale(SingleImageOperation):
...
@ROTATED_PIPELINES.register_module()
class SARRandomCrop(SingleImageOperation):
    def __init__(self):
        super().__init__()
...
@ROTATED_PIPELINES.register_module()
class TeacherStudentMultiBranch(object):
...
SAR_LOCAL_AUGS = {
    'SARGaussianBlur': SARGaussianBlur
}


def build_sar_aug(type, **kwargs):
    return SAR_LOCAL_AUGS[type](**kwargs)
