...
def train_detector(model,
    ...
    if distributed:
        ...
        find_unused_parameters = cfg.get('find_unused_parameters', True)  # 为了res loss而修改
    ...
    if cfg.resume_from:
        ...
        # MMCV 1.x writes ``iter_N.pth`` before incrementing the zero-based
        # runner counter, so its metadata contains N - 1. Continue at N rather
        # than repeating the optimization step already stored in the file.
        match = re.search(r'iter_(\d+)\.pth$', osp.basename(cfg.resume_from))
        if (isinstance(runner, IterBasedRunner) and match is not None
                and runner.iter + 1 == int(match.group(1))):
            runner._iter += 1
            runner._inner_iter += 1
            logger.info('Continue after completed iteration %s.', runner.iter)
    elif cfg.load_from:
