...
class TeacherStudentDetector(RotatedBaseDetector):
    def __init__(self, model: dict, semi_loss_unsup=None, train_cfg=None, test_cfg=None):
        super().__init__()
        self.submodules = list(model.keys())
        for k, v in model.items():
            setattr(self, k, v)
        if semi_loss_unsup is not None:
            self.semi_loss_unsup = build_loss(semi_loss_unsup)    
    ...
    def train(self, mode=True):
        """Set training mode while keeping frozen submodules in evaluation mode."""
        super().train(mode)
        for model_ref in self.submodules:
            model = getattr(self, model_ref)
