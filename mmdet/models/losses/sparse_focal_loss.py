...
def py_sparse_sigmoid_focal_loss(pred,
                                 ...
                                 positive_weight=1.0,
                                 weight_hard_negatives_only=False):
    ...
    # Manuscript Eq. (33) uses the direct condition p(a) > thr.
    hard_negatives = (pred_sigmoid > thresh) & (target == 0)
    ...
    if weight is not None:
        ...
        if weight_hard_negatives_only:
            # In manuscript Eq. (33), the SAL region prior is attached only
            # to high-confidence unmatched (negative) predictions.
            loss = torch.where(hard_negatives, loss * weight, loss)
            weight = None
...
class SparseFocalLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight_hard_negatives_only = weight_hard_negatives_only
    ...
    def forward(self,
        ...
        if self.use_sigmoid:
            ...
            loss_cls = self.loss_weight * py_sparse_sigmoid_focal_loss(
                ...
                weight_hard_negatives_only=self.weight_hard_negatives_only
            )
        else:
