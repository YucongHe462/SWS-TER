...
class DOTADataset(CustomDataset):
    ...
    CLASSES = ('golffield', 'vehicle', 'Expressway-toll-station', 'trainstation',
           'chimney', 'storagetank', 'ship', 'harbor', 'airplane',
           'tenniscourt', 'groundtrackfield', 'dam', 'basketballcourt',
    ...
    def load_annotations(self, ann_folder):
        ...
        if not ann_files:  # test phase
            ann_files = glob.glob(ann_folder + '/*' + self.img_suffix)
            for ann_file in ann_files:
                data_info = {}
                img_id = osp.splitext(osp.split(ann_file)[1])[0]
